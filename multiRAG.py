...
from Image_Processor.update_faiss_with_images import ImageFAISSUpdater
import tiktoken
import PyPDF2
...
class MultiRAG:
    ...
    def __init__(self, 
                 index_path: str = str(CAMPUS_INDEX_DIR),
    ...
    def _process_images(self, source_folder: str) -> List[Dict]:
        """处理文件夹中的图片"""
        """source_folder是文档路径，与Build传入的参数是一致的，这里传入是知识库"""
        print("开始处理图片...")
    ...
    def ensure_image_index(self, source_folder: str) -> None:
        """确保图片已被处理并索引到FAISS，同时生成映射文件。
        ...
        try:
            ...
            if os.path.isdir(self.image_output_dir):
                try:
                    saved_images_count = len([f for f in os.listdir(self.image_output_dir) if os.path.isfile(os.path.join(self.image_output_dir, f))])
                except Exception:
                    saved_images_count = 0

            # 检查向量库中是否有图片描述chunk
            image_chunk_count = 0
            if self.vector_store and getattr(self.vector_store, 'ids', None):
            ...
            if not self._image_mapping:
                print("图片映射为空，准备重新处理图片并建立映射...")
                needs_processing = True
            if saved_images_count == 0:
                print("图片输出目录为空，准备保存处理后的图片...")
                needs_processing = True
            if image_chunk_count == 0:
                print("向量库中没有图片描述，准备添加图片描述到数据库...")
                needs_processing = True

            if not needs_processing:
                print("图片索引与映射已存在，无需处理。")
                return

            # 处理并保存图片
            print("开始处理并索引图片...")
            processed_images = self._process_images(source_folder)
            if not processed_images:
                print("未从文档中提取到任何图片，跳过图片索引。")
                return
            ...
            print(f"ensure_image_index 执行失败: {e}")
...
#--------------------------lg开始修改-------------------------
                    ...
                    # 从json文件中获取图片的文件名(例如:“体育场馆预约的攻略指南.docx_image_1.jpg”)
                    image_file_name = image_info.get('image_filename', '')

                    source_path = "" #初始化为空字符串
                    if image_file_name:
                        # 使用 os.path.join 来拼接路径
                        # self.image_output_dir的值来自于 CAMPUS_IMAGES_PATH
                        full_image_path = os.path.join(self.image_output_dir, image_file_name)
                        if os.path.exists(full_image_path):
                            # 如果存在，就是使用这个路径
                            source_path = full_image_path
                        else:
                            # 如果不存在，就报个错误
                            source_path = f"错误：图片文件在指定位置不存在: {full_image_path}"
                    else:
                        # 如果json中连文件名都没有，也返回错误信息
                        source_path = f"错误，在映射文件中找不到image_id'{image_id}的文件名'"
#------------------------------lg修改完毕----------------------
