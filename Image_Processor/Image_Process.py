...
from Utils.Path import (
    ...
    CAMPUS_PROCESSED_EXTRACTED_IMAGES, FITNESS_PROCESSED_EXTRACTED_IMAGES, 
    PAPER_PROCESSED_EXTRACTED_IMAGES, PSYCHOLOGY_PROCESSED_EXTRACTED_IMAGES,
    CAMPUS_EXTRACTED_IMAGES_JSON, FITNESS_EXTRACTED_IMAGES_JSON,
    PAPER_EXTRACTED_IMAGES_JSON, PSYCHOLOGY_EXTRACTED_IMAGES_JSON,
    CAMPUS_IMAGES_PATH
)
from LLM_library import LLMFactory
...
class ImageExtractor:
    ...
    def enhance_description_with_qwen3(self, image_description: str, context_before: str, context_after: str) -> str:
        ...
        try:
            ...
            system_prompt = 'You are a helpful assistant that analyzes images in context.'
    ...
    def save_images_to_json(self, processed_images: List[Dict], json_output_path: str):
        """将处理后的图片数据保存到JSON文件"""
        try:
...
def process_knowledge_base_images(docs_dir, output_word_path, json_output_path, kb_name=""):
    """处理指定知识库中的图片"""
    print(f"\n开始处理{kb_name}知识库中的图片...")
    
    # 确保输出目录存在
    os.makedirs(os.path.dirname(output_word_path), exist_ok=True)
    
    # 创建图片提取器
    extractor = ImageExtractor(docs_dir)
    ...
    if processed_images:
        ...
        # 保存处理结果到JSON文件
        extractor.save_images_to_json(processed_images, json_output_path)
        
        print(f"{kb_name}知识库图片处理完成！")
        return True
    else:
        print(f"{kb_name}知识库中没有找到任何图片")
        return False

def main():
    """主函数"""
    # 处理校园知识库图片
    process_knowledge_base_images(
        docs_dir=str(CAMPUS_DOCS_DIR),
        output_word_path=str(CAMPUS_PROCESSED_EXTRACTED_IMAGES),
        json_output_path=str(CAMPUS_EXTRACTED_IMAGES_JSON),
        kb_name="校园"
    )
    ...
    process_knowledge_base_images(
        ...
        output_word_path=str(FITNESS_PROCESSED_EXTRACTED_IMAGES),
        json_output_path=str(FITNESS_EXTRACTED_IMAGES_JSON),
        kb_name="健身饮食"
    )
    
    # 处理论文助手知识库图片
    process_knowledge_base_images(
        docs_dir=str(PAPER_DOCS_DIR),
        output_word_path=str(PAPER_PROCESSED_EXTRACTED_IMAGES),
        json_output_path=str(PAPER_EXTRACTED_IMAGES_JSON),
        kb_name="论文助手"
    )
    ...
    process_knowledge_base_images(
        ...
        output_word_path=str(PSYCHOLOGY_PROCESSED_EXTRACTED_IMAGES),
        json_output_path=str(PSYCHOLOGY_EXTRACTED_IMAGES_JSON),
        kb_name="心理助手"
    )
