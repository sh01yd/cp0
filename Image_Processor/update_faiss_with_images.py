...
class ImageFAISSUpdater:
    ...
    def __init__(self, faiss_store_path: str = "faiss_index1"):
        ...
        # 初始化或加载FAISS存储，使用与RAG_compus一致的collection_name
        try:
            self.faiss_store = FAISSVectorStore(index_path=faiss_store_path, collection_name="campus_docs")
            print(f"FAISS存储初始化完成: {faiss_store_path}, collection: campus_docs")
...
def main():
    ...
    # 同时为每条记录生成一个合成的文件名，便于下游路径拼接
    synthesized_images = []
    for idx, img in enumerate(processed_images):
        # 合成文件名：<source_file>_image_<n>.jpg
        src = img.get('source_file', f'unknown_{idx}.docx')
        synth_name = f"{src}_image_{idx+1}.jpg"
        entry = img.copy()
        entry['image_filename'] = synth_name
        # 这里不提供真实的 image_path/processed_path，因为物理文件可能不存在
        synthesized_images.append(entry)

    image_chunks, image_mapping = updater.create_image_chunks(synthesized_images)

    # 保存图片映射文件
    # 保存到项目内的校园映射文件路径，供 MultiRAG 检索使用
