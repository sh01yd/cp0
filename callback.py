...
class LLM_model:
    def __init__(self):
        ...
        # 确保图片已处理并索引，避免问答阶段没有图片输出
        try:
            self.multirag.ensure_image_index(str(CAMPUS_DOCS_DIR))
        except Exception as e:
            print(f"初始化阶段确保图片索引失败: {e}")
    ...
    def retrieve_and_answer(self, query: str, top_k: int = 8):
        ...
        try:
            ...
            # 3. 先将图片信息作为独立块输出，供上层按图片逻辑处理
            for img in image_info:
                try:
                    yield {
                        'type': 1,  # 图片类型标识
                        'description': img.get('description', ''),
                        'source': img.get('path', ''),  # 与上层约定的键名保持一致
    ...
    def call_llm_stream(self, query, list):
        ...
        separator = "\n\n"
        #----------------xkl改start——————————————
        # 将非字符串内容转换为字符串，避免 join 类型错误
        safe_chunks = []
        for item in list:
            if isinstance(item, dict):
                desc = item.get('description') or item.get('document') or ''
                path = item.get('path') or item.get('source') or ''
                if desc and path:
                    safe_chunks.append(f"[图片内容] {desc} [图片地址: {path}]")
                elif desc:
                    safe_chunks.append(f"[图片内容] {desc}")
                else:
                    safe_chunks.append(json.dumps(item, ensure_ascii=False))
            else:
                safe_chunks.append(str(item))
        #----------------xkl改end——————————————
